...
dt_expression = data.table::fread("FANTOM_CAT.expression_atlas.gene.lv3_robust.MEDIAN_tpm_PER_ONTHO_ELYAS.tsv",
                                  stringsAsFactors = F,
                                  drop=1)
```
...
perf_filename='lincrna_PFb_mann_whitney_pval_Full_svc_Intermediary100.tsv' # for 1st plot 
...
j <- plot_ly(x=tsne$Y[,1],
             y=tsne$Y[,2],
             mode="markers+text",
             label=paste(sapply(TSNE$onto, getOntoName),
                        "\n",
                        round(accu[rownames(dt_expression_bis)],2),
                        'accuracy'),
