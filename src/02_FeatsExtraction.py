...
"""
This scripts is used to calculate the fragmentomic features.
        ...
        if IsInt:
            if mn >= 0:
                ...
                if mn > np.iinfo(np.int8).min and\
                        mx < np.iinfo(np.int8).max:
                    props = props.astype(np.int8)
                elif mn > np.iinfo(np.int16).min and\
                        mx < np.iinfo(np.int16).max:
                    props = props.astype(np.int16)
                elif mn > np.iinfo(np.int32).min and\
                        mx < np.iinfo(np.int32).max:
                    props = props.astype(np.int32)
                elif mn > np.iinfo(np.int64).min and\
...
parser.add_argument('-o', '--outfile', type=str,
                    help='Output file for the scores')
parser.add_argument('-iw', '--inwps', type=str,
                    help='Input table of FD scores')
parser.add_argument('-if', '--infd', type=str, help='Input table of FD scores')
parser.add_argument('-v', '--verbose', action='store_true',
                    help='Boolean (default=True) Whether to print\
...
TSS2kFD = score_table.iloc[:, (TSS-1000):(TSS+1000+1)]\
    .apply(statistics.mean, axis=1)
TSS2kFD.name = 'TSS2kFD'
features.append(TSS2kFD)
TSSFD = score_table.iloc[:, (TSS-150):(TSS+50+1)]\
    .apply(statistics.mean, axis=1)
TSSFD.name = 'TSSFD'
features.append(TSSFD)
TSSvar = score_table.iloc[:, (TSS-150):(TSS+50+1)]\
    .apply(statistics.variance, axis=1)
TSSvar.name = 'TSSvar'
features.append(TSSvar)
PseudoVarFD = score_table.iloc[:, (TSS-150):(TSS+50+1)]\
...
if args.verbose:
    print('Scaling FD')
score_table = score_table.iloc[:, :10001].apply(scale, axis=1).apply(
    pd.Series).apply(reduce_mem_usage, axis=0).join(ids)
if args.verbose:
    print('Done\n')
...
TSS2kFD_scaled = score_table.iloc[:, (TSS-1000):(TSS+1000+1)]\
    .apply(statistics.mean, axis=1)
TSS2kFD_scaled.name = 'TSS2kFD_scaled'
features.append(TSS2kFD_scaled)
TSSFD_scaled = score_table.iloc[:, (TSS-150):(TSS+50+1)]\
    .apply(statistics.mean, axis=1)
TSSFD_scaled.name = 'TSSFD_scaled'
features.append(TSSFD_scaled)
TSSvar_scaled = score_table.iloc[:, (TSS-150):(TSS+50+1)]\
    .apply(statistics.variance, axis=1)
TSSvar_scaled.name = 'TSSvar_scaled'
features.append(TSSvar_scaled)
PseudoVarFD_scaled = score_table.iloc[:, (TSS-150):(TSS+50+1)]\
    .apply(lambda x: max(x)-min(x), axis=1)
...
# append feat_table
feat_table = feat_table.merge(score_table.iloc[:, 10001:],
                              on=['geneID',
                                  'chrom',
                                  'strand',
                                  'start',
...
pseudoVarPlus1 = score_table.iloc[:, TSS:(TSS+300+1)]\
    .apply(lambda x: max(x[0:(200+1)])-min(x[(150):(300+1)]), axis=1)
...
if args.verbose:
    print('Extractcting scaled FD features\n')
pseudoVarPlus1_scaled = score_table.iloc[:, TSS:(TSS+300+1)]\
    .apply(lambda x: max(x[0:(200+1)])-min(x[(150):(300+1)]), axis=1)
...
feat_table.iloc[:, 5:] = feat_table.iloc[:,
                                         5:].apply(lambda x: x.astype(float))
