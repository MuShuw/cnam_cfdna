...
def count_depth(chr, size, input):
    """
    Count the depth of the read. For each genomic coordinate return the number
    ...
    """
    bamfile = pysam.AlignmentFile(input, 'rb')
    array_depth = np.zeros(size+1,dtype = np.uint16) #WARNING this might reduce the range of the coverage
...
def parallel_runs(input, list_chromosomes, nb_core):
    t2 = time.time()
    tps = t2 - t1
    print(tps)
...
def get_wps_and_frag(chr, input, size, w_size,frag_min,frag_max):
    ...
    """
    bamfile = pysam.AlignmentFile(input, 'rb')
    frag_overlap = np.zeros(size+1,dtype = np.uint16) #WARNING this might reduce the range of the coverage
    ...
    for read in bamfile.fetch(contig=chr):
        if read.is_proper_pair:
            ...
            if frag_min <= flen <= frag_max:
                ...
                wps_array[int(frag_start -(w_size-1)/2):int(frag_start+flen+(w_size-1)/2+1)] -=1
                if (flen >= w_size):
                    wps_array[int(frag_start +(w_size-1)/2):int(frag_start+flen-(w_size-1)/2)+1] +=2
    bamfile.close()
...
def data_print(chr, size, parameters):
    ...
    frag_array,wps_array = get_wps_and_frag(chr, input, size,w_size,frag_min,frag_max)
    t1 = time.time()
    depth_array,frag_array,wps_array = depth_array[1:,],frag_array[1:,],wps_array[1:,]
    out_matrix = np.rec.fromarrays([depth_array,frag_array,wps_array], names='rd,fd,wps')
    np.savez_compressed(out_name, out_matrix)
    t2 = time.time()
    total_time = round((t2 - timer1)/60)
...
if __name__ == "__main__":
    ...
    print("fragments used for WPS is between {} and {} bp".format(str(args.frag_min),str(args.frag_max)))
    ...
    # parallel_runs(args.input,list_chromosomes,list_length,args.nb_core)
    try :
        #pool = Pool(os.cpu_count() - 1)
        pool = Pool(23)
        dataProcessor = ChromProcessor((args.input, args.wind_size, args.frag_min, args.frag_max))
