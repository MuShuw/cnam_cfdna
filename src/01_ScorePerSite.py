...
parser = argparse.ArgumentParser(description='Sequence Score Table Creation')
...
def extract_strand_start(pd_df_line, pos = 0):
    ...
    to_return=pd.Series([pd_df_line.geneID, chrom[3:], strand, TSS_start], index=['geneID', 'chrom', 'strand', 'start'])
    return(to_return)
...
def check_strand(pd_df_line):
    ...
    """
    to_return = pd_df_line['geneID'], pd_df_line['chrom'], int(pd_df_line['start'])-RANGE, int(pd_df_line['start'])+RANGE+1, pd_df_line['strand']
...
def sort_ranges(range_list):
    ...
    tmp_range_list=pd.DataFrame(tmp_range_series, columns=['range']).assign(min_val= tmp_range_series.apply(min)).sort_values('min_val')['range']
    ...
    for higher in list_range:
        if merged:
            ...
            if min(higher) <= max(lower)+1:
                ...
                merged[-1] = range(min(min(higher), min(lower)), upper_bound+1)  # replace by merged interval
            else:
...
def get_usable_range_withoutX(score_range):
    ...
    for chrom in chroms:
        plop[chrom]=list((score_range[score_range.chrom==chrom]['geneID'].values,
                          score_range[score_range.chrom==chrom]['first_pos'].values,
                          score_range[score_range.chrom==chrom]['last_pos'].values,
                          score_range[score_range.chrom==chrom]['strand'].values))
    # We create the final iterator
    iterators = dict()
    for key in plop.keys():
        iterators[key] = list(zip(plop[key][0], plop[key][1], plop[key][2], plop[key][3]))
...
def extract_for_chr(chrom, iterator):
    try :
        ...
        if len(chrom_files)!=1:
            print("Wrong number of files ending with chr"+chrom+".npz \n Skipping chromosome number"+chrom)
            return()
...
def set_table(score_type, table, chroms):
    saved_score=dict()
    if score_type==1:
        for chrom in chroms:
            saved_score[chrom] = pickle.load(open(FD_PICKLE+chrom+'.pickle', "rb"))
    else:
        for chrom in chroms:
            saved_score[chrom] = pickle.load(open(WPS_PICKLE+chrom+'.pickle', "rb"))
    saved_score = pd.concat(saved_score.values())
    saved_score['geneID'] = table.geneID
    saved_score['geneClass'] = table.CAT_geneClass
    saved_score['chrom'] = table.chrom
    return saved_score
...
def save_table(table, chroms):
    ...
    wps_saved_score.to_csv(PATH_OUT+'/WPS_'+GENE_CLASS+'scoreTable.tsv', sep='\t', index=False)
    fd_saved_score.to_csv(PATH_OUT+'/FD_'+GENE_CLASS+'scoreTable.tsv', sep='\t', index=False)
...
if __name__ == '__main__':
    ...
    cell_type_gene_association.columns=['geneID', 'CAT_geneName', 'CAT_geneCategory',
                                        'CAT_geneClass', 'top_associated_sample_ontology',
                                        'num_associated_sample_ontology', 'CAT_browser_link',
    ...
    robust_major_tss = robust_major_tss.merge(bed_point, on='strongest_DPIClstrID', how='left')
    # We filter the biotype
    geneTypeSelection = cell_type_gene_association[cell_type_gene_association.CAT_geneClass == GENE_CLASS]
    # We join the filtered table with the TSSs table (still not sure why
    # I didn't filtered directly the TSS table)
    detailed_table = geneTypeSelection.merge(robust_major_tss, how='left', left_on='geneID', right_on='geneID')
    ...
    simple_table = simple_table.merge(detailed_table[['geneID', 'CAT_geneClass']], how='left', left_index=True, on='geneID')
